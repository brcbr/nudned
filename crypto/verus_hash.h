...
class CVerusHashV2
{
    public:
        ...
        inline void FillExtra(const T *_data)
        {
            ...
            do
            {
                int len = left > static_cast<int>(sizeof(T)) ? static_cast<int>(sizeof(T)) : left;
        ...
        static u128 *GenNewCLKey(unsigned char *seedBytes32)
        {
            ...
            if (pdesc->seed != *((uint256 *)seedBytes32))
            {
                ...
                if (nbytesExtra)
                {
                    ...
                    std::memcpy(pkey, buf, nbytesExtra);
                }
                pdesc->seed = *((uint256 *)seedBytes32);
                std::memcpy(key + size, key, refreshsize);
            }
            else
            {
                std::memcpy(key, key + size, refreshsize);
            }

            std::memset((unsigned char *)key + (size + refreshsize), 0, size - refreshsize);
            return (u128 *)key;
        }
